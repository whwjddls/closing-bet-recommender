...
class PickResult(BaseModel):
    ...
    vwap_0900_0920: float | None = None     # 판정 청산가(09:00–09:20 VWAP)
...
class ReminderPick(BaseModel):          # S7 익일 오전 청산 리마인더 — 픽별 청산 가이드
    ...
    morning_vwap: float | None = None   # vwap_0900_0920(판정 청산가); 미채점 시 None(UI "채점 대기")
...
class PrefetchTodayResponse(BaseModel):  # GET /prefetch/today — DB 기반 오늘 프리스캔 기록
    """08:30 스케줄러(별도 프로세스) 프리스캔도 반영한다 — ``/jobs/prefetch/status`` 는
    웹서버 프로세스 내부 상태라 스케줄러 실행을 못 봐 항상 '미실행'으로 보인다."""
    ran: bool                           # 오늘 프리스캔 산출물 존재 여부
    ticker_count: int = 0               # FINAL 지표 저장 종목 수
    universe_count: int = 0             # 선정 유니버스 크기
    as_of: str | None = None            # 기준일(YYYY-MM-DD)

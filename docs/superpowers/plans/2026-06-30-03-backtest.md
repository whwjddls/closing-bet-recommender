...
- 서브시스템 2 (signals, 공식은 아키텍처 §2 고정): `compute_s_shin(p_now, high_252, high_60, listing_days)`, `compute_rvol_confirm(rvol)`, `compute_supply_tilt(net_purchase, avg_value_20d)`, `compute_regime_mult(index_level, ma5, ma5_prev)` — `reconstruct.compute_signal_panel(panel, signal_fns)`에 **콜러블 dict로 주입**(Task 12). 테스트는 스텁 콜러블을 주입해 네트워크·미완성 모듈 의존 없이 검증.
